...
def main():
    ...
    if not check_only and os.path.exists(hub):
        ...
        cards = "\n".join(
            '        <a class="article-card article-live" href="%s">\n          <div>\n            <span class="article-type">%s · v%s · %s · n=%s</span>\n            <h3>%s</h3>\n            <p>%s</p>\n          </div>\n          <span class="article-link">Read →</span>\n        </a>'
            % (p["url"], html.escape(p.get("type_label") or p["type"]), p["version"], human_date(p.get("date", "")), p.get("n", ""),
               html.escape(p["title"]), html.escape(p.get("summary", "")))
            for p in pubs)
        block = "<!-- research-list:start (generated by scripts/build_research.py) -->\n" + cards + "\n        <!-- research-list:end -->"
        m = re.search(r'(<div class="article-list" data-research-list[^>]*>)(.*?)(</div>\s*\n)', hb, re.S)
        if m:
            inner = m.group(2)
