...
<head>
  ...
  <script src="../js/findings.js?v=2" defer></script>
...
def main():
    ...
    for name in sorted(os.listdir(SRC)):
        ...
        _url = "%s/research/%s.html" % (SITE, slug)
        _ld = {
            "@context": "https://schema.org",
            "@type": "ScholarlyArticle" if meta.get("type") in ("white-paper", "brief") else "Article",
            "@id": _url, "url": _url, "mainEntityOfPage": _url,
            "headline": meta.get("title", slug),
            "alternativeHeadline": meta.get("subtitle", "") or None,
            "description": meta.get("summary", ""),
            "version": str(meta.get("version", "1.0")),
            "author": {"@type": "Person", "@id": SITE + "/about.html#nima", "name": meta.get("author", "Nima Imani"), "url": SITE + "/about.html"},
            "publisher": {"@type": "Organization", "@id": SITE + "/#organization", "name": "InsightsOut", "url": SITE + "/", "logo": {"@type": "ImageObject", "url": SITE + "/assets/logo.svg"}},
            "datePublished": meta.get("first_published", meta.get("date", "")),
            "dateModified": meta.get("date", ""),
            "inLanguage": "en",
            "isPartOf": {"@id": SITE + "/#website"},
            "about": ["AI change", "human side of AI adoption", "role change", "founder overwhelm", "leadership"],
            "isAccessibleForFree": True,
        }
        if pdf_rel:
            _ld["encoding"] = {"@type": "MediaObject", "encodingFormat": "application/pdf", "contentUrl": "%s/%s" % (SITE, pdf_rel)}
        _ld = {k: v for k, v in _ld.items() if v not in (None, "")}
        page = TEMPLATE.format(
            jsonld=json.dumps(_ld, indent=2, ensure_ascii=False),
    ...
    # insights.html: write static cards into the research-list mount (SEO + works without JS/fetch).
    hub = os.path.join(ROOT, "insights.html")
    if not check_only and os.path.exists(hub):
        with open(hub, encoding="utf-8") as f:
            hb = f.read()
        pubs = [p for p in manifest["publications"] if p["status"] != "draft"]
        pubs.sort(key=lambda p: (p.get("weight") or 0, p.get("date", "")), reverse=True)
        def _card(p):
            pdf = ('<a class="card-cta" href="%s">PDF ↓</a>' % p["pdf"]) if p.get("pdf") else ""
            sub = ('\n          <p class="paper-sub">%s</p>' % html.escape(p["subtitle"])) if p.get("subtitle") else ""
            return (
                '        <article class="card paper-card">\n'
                '          <span class="offer-kicker">%s · v%s · %s · n=%s</span>\n'
                '          <h3><a href="%s">%s</a></h3>%s\n'
                '          <p>%s</p>\n'
                '          <div class="paper-actions"><a class="btn btn-primary btn-sm" href="%s">Read the paper</a>%s</div>\n'
                '        </article>'
            ) % (html.escape(p.get("type_label") or p["type"]), p["version"], human_date(p.get("date", "")), p.get("n", ""),
                 p["url"], html.escape(p["title"]), sub, html.escape(p.get("summary", "")), p["url"], pdf)
        cards = "\n".join(_card(p) for p in pubs)
        block = "<!-- research-list:start (generated by scripts/build_research.py) -->\n" + cards + "\n        <!-- research-list:end -->"
        m = re.search(r'(<div class="(?:article-list|paper-grid)" data-research-list[^>]*>)(.*?)(</div>\s*\n)', hb, re.S)
        if m:
            inner = m.group(2)
            if "research-list:start" in inner:
                inner_new = re.sub(r"<!-- research-list:start.*?research-list:end -->", block, inner, flags=re.S)
            else:
                inner_new = "\n        " + block + "\n      "
            hb2 = hb[:m.start(2)] + inner_new + hb[m.end(2):]
            if hb2 != hb:
                with open(hub, "w", encoding="utf-8") as f:
                    f.write(hb2)
                print("  insights.html: research list updated (%d cards)" % len(pubs))
    if problems:
